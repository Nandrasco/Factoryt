...
describe('Salle e2e test', () => {
    ...
    it('should load Salles', async () => {
        ...
        expect(await salleComponentsPage.getTitle()).to.eq('jhipsterSampleApplicationApp.salle.home.title');
    });

    it('should load create Salle page', async () => {
        await salleComponentsPage.clickOnCreateButton();
        salleUpdatePage = new SalleUpdatePage();
        expect(await salleUpdatePage.getPageTitle()).to.eq('jhipsterSampleApplicationApp.salle.home.createOrEditLabel');
    ...
    it('should create and save Salles', async () => {
        ...
        const selectedDispo = salleUpdatePage.getDispoInput();
        if (await selectedDispo.isSelected()) {
            await salleUpdatePage.getDispoInput().click();
            expect(await salleUpdatePage.getDispoInput().isSelected()).to.be.false;
        } else {
            await salleUpdatePage.getDispoInput().click();
    ...
    it('should delete last Salle', async () => {
        ...
        expect(await salleDeleteDialog.getDialogTitle()).to.eq('jhipsterSampleApplicationApp.salle.delete.question');
